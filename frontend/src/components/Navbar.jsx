  ...
  return (
    <header>
      ...
      <div className="bg-blue-900 text-white flex flex-wrap py-3 px-4 justify-between items-center">
        <div className="flex flex-wrap items-center space-x-4 text-white text-[12px]">
          <FaMapMarkerAlt size={20} color="black" className="ml-2" />
          <p>Balangoda, Sabaragamuwa Province</p>
          <FaPhoneAlt size={20} color="black" />
          <p>+94 11223344</p>
          <FaSchool size={20} color="orange" />
          <p>Instruct with LEARNING DRIVING SCHOOL</p>
        </div>
        <div className="flex space-x-4 mt-2 md:mt-0">
          <Link to="/login">
            <button
              className={`py-1 px-3 rounded-full font-medium ${
                activePage === 'learner' ? 'bg-gray-800 text-white' : 'bg-white text-blue-900'
              }`}
          ...
          <Link to="/InstLogin">
            <button
              className={`py-1 px-3 rounded-full font-medium ${
                activePage === 'instructor' ? 'bg-gray-800 text-white' : 'bg-white text-blue-900'
              }`}
